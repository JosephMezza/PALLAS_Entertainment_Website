...
$fname = $lname = $email = $country = $street_address = $city = $zip = $card_type = $card_number = $cardholder = $expiration_date = $cvv = $ticket_quantity = $amount = "";
$fname_err = $lname_err = $email_err = $country_err = $street_address_err = $city_err = $zip_err = $card_type_err = $card_number_err = $cardholder_err = $expiration_date_err = $cvv_err = $amount_err = "";
$dash = "-";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } else if (!(is_numeric($_POST["card_number"])) && preg_match('/-/', $_POST["card_number"]) == false) {
        $card_number_err = "Please enter a number.";
    } //Checks if length is correct
    elseif (strlen($_POST["card_number"]) != 19) {
        $card_number_err = "Invalid card number length";
    } // Checking if dashes are properly placed
    elseif (($_POST["card_number"][4] != $dash) && ($_POST["card_number"][9] != $dash)  && ($_POST["card_number"][14] != $dash)) {
        $card_number_err = "Invalid card format";
    } else {
    ...
    if ($_POST["ticket_quantity"] < 1) {
        $amount_err = "Please select how many tickets you would like to purchase";
    }



    // Check input errors before inserting in database
    if (empty($fname_err) && empty($lname_err) && empty($email_err) && empty($card_number_err) && empty($cardholder_err) && empty($street_address_err) && empty($expiration_date_err) && empty($cvv_err) && empty($city_err) && empty($zip_err) && empty($amount_err)) {
         ...
         */
        $tax = $price * 0.15; //Tax needs to be created here since it is not an input box in the form
        $subtotal = $price + $tax;
        ...
        $tax = $tax . "$"; //Dollar signs are concatenated after the arithmetic is completed.
        $subtotal = $subtotal . "$";
...
<head>
    ...
    <script type="text/javascript">
        //Function which increments number of tickets
        function increment() {
            var price = <?php echo $price ?>; //Store session price
            var value = parseInt(document.getElementById("ticket_quantity").value);
            //If the value is null,then change it to 1 once its incremented.
            value = isNaN(value) ? 1 : value;
            value++; //The value which will later be
            document.getElementById("ticket_quantity").value = value;
            var newPrice = value * price;
            //Calculates price based on number of tickets
            newPrice = Math.round(newPrice, 2);
            document.getElementById("dynamicPrice").innerHTML = newPrice + "$";
            var tax = newPrice * .15; //Calculates tax
            var subtotal = newPrice + tax; //Calculates subtotal (price + tax)
            tax = Math.round(tax, 2);
            subtotal = Math.round(subtotal, 2);
            document.getElementById("tax").innerHTML = tax + "$"; //Outputs the tax price to the html
            document.getElementById("subtotal").innerHTML = subtotal + "$"; //Outputs the subtotal to the html

        }

        function decrement() {
            var price = <?php echo $price ?>; //Store session price
            var value = parseInt(document.getElementById("ticket_quantity").value); //Retrieve the ticket quantity from the input box when the decrement picture is clicked
            value = isNaN(value) ? 0 : value; //If the value is null, set it to 0
            value--;
            if (value <= 1) { //If the vaue is greater 0 or less set it to one, prevents people from buying 0 or less tickets
                value = 1;
            }
            var newPrice = value * price; //Price calculation is the same as above
            document.getElementById("ticket_quantity").value = value;
            document.getElementById("dynamicPrice").innerHTML = newPrice + "$";
            newPrice = Math.round(newPrice, 2);
            var tax = newPrice * .15;
    ...
    <style>
        ...
        .checkout {
            ...
            /* Adds the same pattern from the navbar to the background of the checkout form */
            border-radius: 10px;
...
<body>
    ...
    <div class="checkout">
        <!-- This form includes a table which is used to take all user inputs. The name of every input is stored in a session variable which is sent to MailTest.php, allowing the data to be sent in an email -->
            ...
            <table>
                ...
                <tr>
                    ...
                    <td rowspan=5>
                        <table>
                            <tr>
                                <!-- The increment/decrement buttons are included here with onclick functions, linking them to the javascript above -->
                            ...
                            <tr>
                                <td colspan="3">Tax: <p id="tax" name="tax" value="0"></p><!-- Javascript functions write to these three <p> tags dynamically (increment/decrement) -->
                                </td>
                            </tr>
                            <tr>
                                <td colspan="3">Subtotal: <p id="subtotal" name="subtotal" value="0"></p>
                                </td>
                            </tr>

                            <tr>
                                <td colspan="3"><button style="cursor: pointer;">Checkout</button></td>
                            </tr>
                ...
                <tr>
                    ...
                    <td colspan="2">
                        <fieldset class="card">
                            ...
                            <input class="none" type='text' placeholder="Format: 1234-5678-9123-4567" name=card_number>
                            <br />
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <br>
                        <input type="text" name="postal_code" placeholder="Format: A3J4-b7U8" value="<?php echo $zip; ?>">
                        <br />
